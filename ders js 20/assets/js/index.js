       ...
       <div class="inbox">
                        ...
                        <div class="text">
                            ...
                            <div class="boxs">
                                ...
                                <div class="box">
                                    <i onclick="addtoCard(${item.id})" class="fa-solid fa-cart-shopping"></i>
                                </div>
                                <div class="box">
                                    <i onclick="wishfunc(${item.id})" class="fa-solid fa-heart"></i>
                                </div>
                            </div>
                        </div>
                    </div>
...
async function addtoCard(id) {
  ...
  try {
    ...
    addcart.innerHTML=`
        <a href="./assets/pages/addtoCard.html"><i class="fa-solid fa-cart-shopping"></i></a>[ ${cart.length} ]
    `  
...
async function wishfunc(id) {
  ...
  try {
    if(id){
      let {data} = await axios.get("https://655f2b37879575426b44b8f7.mockapi.io/productss");  
      let resalt = data.find((item) => item.id == id);
      let resalt1 = wis.find((item) => item.id == id)
      if(resalt1){
        alert("bu mehsul var") 
      }else{
        if (resalt) {
          wis.push(resalt)
          localStorage.setItem("wishlist",JSON.stringify(wis))         
          alert("Mehsul elave edildi") 
        } 
    ...
    heart.innerHTML=`
        <a href="./assets/pages/wishlist.html"><i class="fa-solid fa-heart"></i></a>[ ${wis.length} ]
    `  
